...
frequentItems = eclat (Groceries, parameter = list(supp = 0.01, minlen=1 , maxlen = 5)) #support of 0.01 times the total no of bills which is taking items sold atleast 98.35 times, len takes limits of items in grocery list to avoid comp from hanging
...
rules = apriori(Groceries, parameter = list(supp = 0.005, conf = 0.5, minlen=2))  #conf means that confidence level between IV and DV; apriori is creating rules i.e. x to y which has a confidence level associated with it
rules
inspect (rules[1:5])
